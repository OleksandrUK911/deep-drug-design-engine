...
from ml.tracking import log_artifact, log_metrics, mark_best, tracked_run
...
def main() -> int:
    ...
    valid_loader = DataLoader(valid_graphs, batch_size=args.batch_size)
    test_loader = DataLoader(test_graphs, batch_size=args.batch_size)

    model = build_model(args.architecture, in_dim=NUM_ATOM_FEATURES, hidden_dim=args.hidden_dim, num_layers=args.num_layers).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    loss_fn = torch.nn.BCEWithLogitsLoss() if args.task == "classification" else torch.nn.MSELoss()

    params = vars(args) | {"num_atom_features": NUM_ATOM_FEATURES}
    with tracked_run(f"gnn_property_{args.task}", f"{args.architecture}_{args.dataset.stem}", params, args.dataset) as run:
        ...
        for epoch in range(args.epochs):
            ...
            valid_metrics = evaluate(model, valid_loader, args.task, device)
            key_metric = valid_metrics["roc_auc"] if args.task == "classification" else -valid_metrics["rmse"]
        ...
        test_metrics = evaluate(model, test_loader, args.task, device)
